...
def split_time_series(series, n):
    ...
    Parameters:
        series (list or np.array): The input time series.
        n (int): The length of each segment.

    Returns:
        np.array: Array containing all valid segments of length n.
    """
    # Standard split into segments of size n
...
G = {k: v for k, v in G.items() if len(v)>7}
print("Gruppi G")
print(G)
