...
conf_interval = norm.interval(0.95, mu, std)
# Creare il grafico
...
# Evidenziare l'area sotto la curva per l'intervallo di confidenza al 95%
x_fill = np.linspace(conf_interval[0], conf_interval[1], 100)
y_fill = norm.pdf(x_fill, mu, std)
plt.fill_between(x_fill, y_fill, alpha=0.3, color='green')

# Aggiungere etichette e titolo
plt.title('Distribuzione dei valori di chiusura e intervallo di confidenza al 95%')
plt.xlabel('Valori di chiusura')
plt.ylabel('Densità')

# Aggiungere il testo del risultato dell'ipotesi in alto a destra
plt.text(xmax - (xmax-xmin)*0.3, max(p)*0.9, f'Intervallo 95%: [{conf_interval[0]:.2f}, {conf_interval[1]:.2f}]', fontsize=12, bbox=dict(facecolor='white', alpha=0.6))
