...
for i in range(len(A)):
    for j in range(len(A)):
        ...
        S[i, j] = compute_correlazione(A[i], A[j],A_CHIUSURE[i], A_CHIUSURE[j],degree)
...
for i in range(len(S)):
    ...
    for j in range(len(S)):
        if S[i, j] > abs(THRESHOLD) and i != j and (i, j) not in G and (j, i) not in G and j not in G[i] and len(G[i])<5:
            already_added = False
            for key in G:
                if j in G[key]:
                    already_added = True
...
def correlation_loss(y_true, y_pred):
    """Calcola 1 - correlazione di Pearson come funzione di perdita"""
    y_true_flat = np.array(flatten_list(y_true))
    y_pred_flat = np.array(flatten_list(y_pred))
    
    mean_y_true = np.mean(y_true_flat)
    mean_y_pred = np.mean(y_pred_flat)
    
    y_true_centered = y_true_flat - mean_y_true
    y_pred_centered = y_pred_flat - mean_y_pred
    
    covariance = np.sum(y_true_centered * y_pred_centered)
    std_y_true = np.sqrt(np.sum(y_true_centered ** 2))
    std_y_pred = np.sqrt(np.sum(y_pred_centered ** 2))
    
    pearson_correlation = covariance / (std_y_true * std_y_pred)
    
    # Modifichiamo la funzione di perdita: 1 - Pearson correlation
    return 1 - pearson_correlation

def adjust_learning_rate(epoch, initial_lr, decay_rate=0.96, decay_step=100):
    """Diminuisce il tasso di apprendimento man mano che l'allenamento procede"""
...
def train_rnn(X_train, y_train, DIM_INPUT, DIM_HIDDEN, DIM_OUTPUT, epochs=10, learning_rate=0.001):
    ...
    loss = 100
    epoch = 0
    min_loss_threshold = 0.001  # Soglia più piccola per la convergenza
    
    while loss > min_loss_threshold and epoch < 100:
        y_pred, h = rnn_forward(X_train, W_xh, W_hh, W_hy, b_h, b_y)

        # Usa la correlazione come funzione di perdita (1 - Pearson correlation)
        loss = correlation_loss(y_train, y_pred)
        
        # Stampa il valore della correlazione per capire il progresso
        #print(f'Epoch {epoch+1}, Loss (1 - Correlation): {loss:.6f}')
        
        # Aggiornamento dei pesi con backpropagation
        rnn_backward(X_train, y_train, y_pred, h, W_xh, W_hh, W_hy, b_h, b_y, learning_rate)
        
        #learning_rate = adjust_learning_rate(epoch, initial_lr=learning_rate)
        epoch += 1
...
for k in G.keys():
    dtw_value = compute_correlazione(ULTIMASEQ, A[k], ULTIMASEQ_CHIUSURE, A_CHIUSURE[k],degree)
