...
some_months_ago = today - relativedelta(months=6)
...
# Calcolare media e deviazione standard della distribuzione normale
mu, std = norm.fit(Y)

# Valore di soglia per il test di ipotesi
#soglia = 94  # Sostituire con il valore desiderato
soglia = Y[-1]
# Calcolare la probabilità cumulativa per la soglia
p_value = norm.cdf(soglia, mu, std)

# Livello di significatività
alpha = 0.05

# Determinare se l'ipotesi viene accettata o meno
if p_value < alpha:
    ipotesi_testo = "Ipotesi rifiutata"
else:
    ipotesi_testo = "Ipotesi accettata"

# Creare il grafico
sns.set(style="whitegrid")
plt.figure(figsize=(10, 6))

# Tracciare la distribuzione normale
sns.histplot(Y, kde=True, stat="density", linewidth=0)

# Aggiungere la linea della distribuzione normale
xmin, xmax = plt.xlim()
x = np.linspace(xmin, xmax, 100)
p = norm.pdf(x, mu, std)
plt.plot(x, p, 'k', linewidth=2)

# Evidenziare l'area sotto la curva per Y < soglia
x_fill = np.linspace(xmin, soglia, 100)
y_fill = norm.pdf(x_fill, mu, std)
plt.fill_between(x_fill, y_fill, alpha=0.3, color='red')

# Aggiungere etichette e titolo
plt.title(f'Test di ipotesi unilaterale sinistro per Y < {soglia}')
plt.xlabel('Valori di chiusura')
plt.ylabel('Densità')

# Aggiungere il testo del risultato dell'ipotesi
plt.text(xmax - (xmax-xmin)*0.3, max(p)*0.9, f'{ipotesi_testo}\n(p-value = {p_value:.4f})', fontsize=12, bbox=dict(facecolor='white', alpha=0.6))
plt.show()
