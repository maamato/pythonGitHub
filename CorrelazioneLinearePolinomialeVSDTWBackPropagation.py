...
def rnn_forward(X, W_xh, W_hy, b_h, b_y):
    N = len(X)
    DIM_INPUT = len(X[0])
...
def rnn_backward(X, y_true, y_pred, h, W_xh, W_hy, b_h, b_y, learning_rate):
    N = len(X)
    DIM_INPUT = len(X[0])
    DIM_HIDDEN = len(W_xh[0])
    ...
    for t in reversed(range(N)):
        ...
        for j in range(DIM_HIDDEN):
            dh[j] = sum(dy[k] * W_hy[j][k] for k in range(DIM_OUTPUT))
...
def train_rnn(X_train, y_train, DIM_INPUT, DIM_HIDDEN, DIM_OUTPUT):
    W_xh, W_hy, b_h, b_y = init_rnn_weights(DIM_INPUT, DIM_HIDDEN, DIM_OUTPUT)
    loss=100
    learning_rate=0.1
    epochs=1000
    epoch=0
    #for epoch in range(epochs):
    while loss>0.005 and epoch<1000:
        y_pred, h = rnn_forward(X_train, W_xh, W_hy, b_h, b_y)
        loss = mean_squared_error(y_train, y_pred)
        rnn_backward(X_train, y_train, y_pred, h, W_xh, W_hy, b_h, b_y, learning_rate)
        print(f'Epoch {epoch+1}/{epochs}, Loss: {loss}, lr {learning_rate}')
        epoch=epoch+1
...
for k, v in G.items():
    ...
    # Addestramento della RNN
    val_W_xh, val_W_hy, val_b_h, val_b_y = train_rnn(TargetA, TargetB, input_dim, hidden_units, output_dim)
    #W_xh.append(val_W_xh)
    #W_hy.append(val_W_hy)
    #b_h.append(val_b_h)
    trained_rnn_params=val_W_xh, val_W_hy, val_b_h, val_b_y 
    RNNItems[k]=trained_rnn_params

def check_sequence_belongs_to_network(sequence, sequenceTest, chiusureB, trained_rnn_params):
    W_xh, W_hy, b_h, b_y = trained_rnn_params
    output, _ = rnn_forward([sequence], W_xh, W_hy, b_h, b_y)
...
for k, v in G.items():
    ...
    for key, value in RNNItems.items():
        ...
        sumst=0
        for seq in v:
            out,trend=check_sequence_belongs_to_network(A[seq],B[seq], B_CHIUSURE[seq],RNNItems[key])
            sumout= sumout+out
            if trend=='salita':
                sumup=sumup+1
            if trend=='discesa':
...
for k in G.keys():
    ...
    out,trend=check_sequence_belongs_to_network(A[k],B[k], B_CHIUSURE[k],RNNItems[k])
